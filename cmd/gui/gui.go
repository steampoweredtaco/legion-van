...
func NewMainApp(title string) *MainApp {
...
func GrabMonkey(publicAddr bananoutils.Account) io.Reader {
	...
	defer response.Body.Close()
	copy, _ := io.ReadAll(response.Body)
	bs, err := legion.ConvertSvgToBinary(copy, legion.ImageFormat("PNG"), 1000)
	if err != nil {
		panic(err)
	}
...
func main() {
	...
	img, _, err := image.Decode(GrabMonkey(accounts[0]))
	if err != nil {
		log.Fatal(err)
	}
	guiApp.SetPreview1(img)
	img2, _, _ := image.Decode(GrabMonkey(accounts[1]))
	guiApp.SetPreview2(img2)
	img3, _, _ := image.Decode(GrabMonkey(accounts[2]))
	guiApp.SetPreview3(img3)
	img4, _, _ := image.Decode(GrabMonkey(accounts[3]))
	guiApp.SetPreview4(img4)
	defer func() {
		if r := recover(); r != nil {
			guiApp.ForceQuit()
			log.Error(r)
